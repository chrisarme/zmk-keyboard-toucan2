...
Pillow image, while presenting their actual appearance on the keyboard. Converted opaque
black and white pixels use the intentionally inverted artwork polarity; transparent and
white background pixels default to black.
...
- `--background black|white` selects how transparency and `contain` padding are flattened.
  The default is white which, under the inverted artwork polarity, makes transparent pixels
  and letterboxing black on the physical display.
...
- `--dither floyd-steinberg` preserves the appearance of grayscale photographs with a
  ...
  art.
- `--invert` swaps the processed filled and unfilled pixels.
